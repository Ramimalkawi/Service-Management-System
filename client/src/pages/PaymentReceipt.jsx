...
const PrintableContent = ({ ticket, payment }) => {
  if (!ticket || !payment) {
    return null;
  }
        ...
        <div className="receipt-container">
          ...
          <div className="receipt-details">
            ...
            <div className="receipt-section">
              ...
              <p>
                <strong>Payment ID:</strong> {ticket.location}
                {ticket.ticketNum}
              </p>
              <p>
                <strong>Payment Date:</strong>{" "}
                {payment.paymentDate?.toDate().toLocaleString()}
              </p>
              <p>
...
const PaymentReceipt = () => {
  const { ticketId, paymentId } = useParams();
  ...
  useEffect(() => {
    const fetchReceiptData = async () => {
      ...
      try {
        ...
        const paymentRef = doc(db, "tickets", ticketId, "payments", paymentId);
        const paymentSnap = await getDoc(paymentRef);
        if (paymentSnap.exists()) {
          setPayment(paymentSnap.data());
        }
      } catch (error) {
    ...
    image.onload = () => {
      ...
      if (printWindow) {
        const contentString = ReactDOMServer.renderToString(
          <PrintableContent ticket={ticket} payment={payment} />
        );
  ...
  const handleDownloadPdf = () => {
    ...
    html2canvas(input, { useCORS: true, scale: 2 }).then((canvas) => {
      ...
      pdf.save(`receipt-${ticket.ticketNum}-${paymentId}.pdf`);
    });
  };
